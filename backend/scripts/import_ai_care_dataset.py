...
def find_default_dataset():
    candidates = [
        BASE_DIR / "data" / "AI_CARE_Database.xlsx",
        BASE_DIR.parent / "database" / "AI_CARE_Database.xlsx",
        BASE_DIR / "AI_CARE_Database.xlsx",
...
def clear_database():
    """Wipe out sample/existing data before importing fresh dataset."""
    print("Clearing existing data from database...")
    FallHistory.query.delete()
    Notification.query.delete()
    MedicineSchedule.query.delete()
    HealthRecord.query.delete()
    Medicine.query.delete()
    User.query.delete()
    db.session.commit()
    print("All sample/existing data cleared.")


def import_dataset(path, reset=True):
...
def main():
    ...
    parser.add_argument("xlsx_path", nargs="?", default=None, help="Path to AI_CARE_Database.xlsx")
    parser.add_argument("--no-reset", action="store_true", help="Do not clear existing data before import")
    args = parser.parse_args()
    
    path = Path(args.xlsx_path) if args.xlsx_path else find_default_dataset()

    if not path.exists():
        raise FileNotFoundError(f"Dataset not found at any expected location: {path}")

    print(f"Loading dataset from: {path}")

    try:
        result = import_dataset(path, reset=not args.no_reset)
