...
function MedicineForm({ addMedicine, updateMedicine, handleClose, selectedMedicine }) {
  ...
  const [time, setTime] = useState(selectedMedicine?.time || selectedMedicine?.take_time || "08:00");
  ...
  const handleSubmit = (event) => {
    ...
    const formattedTime = time || "08:00";

    if (!name.trim() || !dosage.trim()) {
      alert("Vui lòng nhập đầy đủ tên thuốc và liều lượng!");
      return;
    }
